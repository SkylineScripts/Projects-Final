...
function Range() {
  ...
  let random = Math.floor(Math.random() * parseInt(Math.abs(range2 - range1), 10)) * PosOrNeg + range1;
    if (random % 2 === 0) {
      if (random > parseInt(range1, 10) && random < parseInt(range2, 10)) {
...
document.querySelectorAll('.btn').forEach((item) => {
  ...
  item.addEventListener("click", (event) => {
    if (item.classList.contains('increase')) {
      ...
      totalClicks.innerHTML = total++;
      totalClicks.textContent = total;
    } else if (AutoValue.textContent == "Autoclick") {
